...
class ReviewControllerTest {
    ...
    @Test void getAll() throws Exception { when(reviewService.findAll()).thenReturn(List.of(new Review())); mockMvc.perform(get("/api/v1/reviews")).andExpect(status().isOk()); }
    @Test void getById() throws Exception { when(reviewService.findById(1)).thenReturn(Optional.of(new Review())); mockMvc.perform(get("/api/v1/reviews/1")).andExpect(status().isOk()); }
    @Test void create() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/reviews").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(r))).andExpect(status().isCreated());
    }
    @Test void deleteById() throws Exception { doNothing().when(reviewService).deleteById(1); mockMvc.perform(delete("/api/v1/reviews/1")).andExpect(status().isNoContent()); }
